...
export default function MemberPickerModal({
  ...
  // //console.log(alreadySelected,'already seleced......')
  // const [selectedMembers,setSelectedMemebers] = useState(alreadySelected)
  ...
  // useEffect(()=>{
  //   setSelectedMemebers(alreadySelected)
  // },[alreadySelected])
  ...
  const toggleSelected = (user) => {
    if (checkIfSelected(user)) {
      if (navigationFrom !== 'Add') {
        Alert.alert("You cann't uncheck member")
      } else {
        setSelected((prev) => {
          const copy = [...prev]
  ...
  useEffect(() => {
    ...
    api.user
      ...
      .then((res) => {
        ...
        // //console.log(selected,'selected members))))))))))))')

        // if(alreadySelected){
        //   //console.log(alreadySelected, '+++++++++++++')

        //     let totaluser = res?.members;
        //     let copy =    totaluser.filter((user1) =>
  ...
  const memberAssign = () => {
    ...
    //   Alert.alert("Must be selected at least 1 Memebers")
    //   return
    // }
