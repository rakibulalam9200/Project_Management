...
const ProjectCustomizationModal = ({
  openModal,
  setOpenModal,
  expandView,
  setExpandView,
  ...
  console.log(expandView,'expand view........')
  const [isdraggable, setIsDraggable] = useState(false)
  const [viewName, setViewName] = useState('collapseView')
  const closeModal = () => {
    setOpenModal(false)
  }
        ...
        <View style={[styles.modal]}>
          <View style={[styles.modalContent]}>
            <View style={{ flex: 3 }}>
              ...
              <TouchableOpacity
                onPress={() => {
                  viewName === 'collapseView'
                    ? setViewName('expandView')
                    : setViewName('collapseView')
                }}
                style={{ alignItems: 'flex-end' }}
              >
                <Text style={[g.body1, styles.viewButton]}>
                  {viewName === 'collapseView' ? 'Expand View' : 'Collapse View'}
                </Text>
              </TouchableOpacity>
              {viewName === 'collapseView' && (
                <DragAndDrop
                  ...
                  renderZone={(zone, children, hover) => {
                    ...
                    return (
                      <View>
                        <Text style={[g.body3, { color: colors.NORMAL, marginBottom: 8 }]}>
                          {zone.text}
                        </Text>
                        <Text style={[g.body2, { color: colors.NORMAL, marginBottom: 8 }]}>
                          {'Visible'}
                        </Text>
                        {children}
                        <View
              ...
              {viewName === 'expandView' && (
                <DragAndDrop
                  ...
                  zoneKeyExtractor={(zone) => zone.id}
                  zones={expandView}
                  items={moreExpandView}
                  itemsContainerStyle={[
                    {flex:1}
                    // !itemView.length ? { paddingVertical: 32 } : { paddingVertical: 8 },
                  ]}
                  onMaj={(expandView, moreExpandView) => {
                    setExpandView(expandView)
                    setMoreExpandView(moreExpandView)
                  }}
                  itemsInZoneStyle={[{ width: '100%', flex: 1 }]}
                  renderZone={(zone, children, hover) => {
                    zone?.items?.map((item) => {
                      ...
                      <View>
                        ...
                        >
                          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            {item?.expand ? <MinusBlueFillIcon /> : <PlusBlueFillIcon />}
                            <View style={{ marginLeft: 10 }}>
                              <Text style={{ fontSize: 18, lineHeight: 21 }}>{item?.label}</Text>
                            </View>
                          </View>
            ...
            <View style={[styles.listItemContainer, { width: '100%', marginVertical: 10 }]}>
              ...
              <CButton
                ...
                onPress={() => {
                  closeModal()
                  setItem('projectItemView', itemView)
                  setItem('projectCollapseView', collapseView)
                  setItem('projectExpandView', expandView)
                  setItem('projectMoreExpandView', moreExpandView)
...
export default ProjectCustomizationModal
